...
export async function contractRoutes(fastify: FastifyInstance) {
  // Route PDF enregistrée dans un plugin enfant ISOLÉ (avant le hook global)
  // afin de ne pas hériter de requireSuperAdmin et gérer l'auth manuellement
  fastify.register(async (child: FastifyInstance) => {
    child.get('/:id/pdf', async (request: FastifyRequest<{ Params: { id: string }; Querystring: { token?: string } }>, reply: FastifyReply) => {
      ...
      const addTextToPage = (page: any, text: string, startY: number, isBold = false) => {
        ...
        for (const word of words) {
          ...
          if (width > contentWidth && line) {
            ...
            line = word;
            y -= lineHeight;
            if (y < margin) return { y, needNewPage: true, remainingText: words.slice(words.indexOf(word)).join(' ') };
          } else {
            line = testLine;
          }
        }
        if (line) { page.drawText(line, { x: margin, y, size: fontSize, font: currentFont, color: rgb(0, 0, 0) }); y -= lineHeight; }
      ...
      page.drawText('CONTRAT D\'APPORTEUR D\'AFFAIRES', { x: margin, y, size: 16, font: fontBold, color: rgb(0, 0, 0) });
      y -= 30;

      const lines = contractContent.split('\n');
      for (const line of lines) {
        if (y < margin + 50) { page = pdfDoc.addPage([pageWidth, pageHeight]); y = pageHeight - margin; }
      ...
      if (contract.status === 'SIGNED' && contract.signedAt) {
        ...
        y -= lineHeight;
        if (contract.signatureIP) { page.drawText(`Adresse IP : ${contract.signatureIP}`, { x: margin, y, size: fontSize, font, color: rgb(0, 0, 0) }); y -= lineHeight; }
        y -= 20;
