...
export async function vendorContractRoutes(fastify: FastifyInstance) {
  // Route PDF dans plugin enfant ISOLÉ (enregistré avant le hook global)
  // afin de ne pas hériter de requireVendor et gérer l'auth manuellement
  fastify.register(async (child: FastifyInstance) => {
    child.get('/contracts/:id/pdf', async (request: FastifyRequest<{ Params: { id: string }; Querystring: { token?: string } }>, reply: FastifyReply) => {
      ...
      const addTextToPage = (page: any, text: string, startY: number, isBold = false) => {
        ...
        for (const word of words) {
          ...
          if (width > contentWidth && line) {
            ...
            line = word;
            y -= lineHeight;
            if (y < margin) return { y, needNewPage: true };
          } else {
            line = testLine;
          }
        }
